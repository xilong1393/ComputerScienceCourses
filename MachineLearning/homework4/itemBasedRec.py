...
def test(dataMat, test_ratio, estMethod, simMeas=pearsonSim):
    ...
    MAE=0
    #set a count to save execution time
    count=int(len(dataMat)/50)
    for user in range(count):
        err=cross_validate_user(dataMat,user,test_ratio,estMethod,simMeas)
        print("the MAE for user ", user, " is ", err[0])
        MAE+=err[0]
    print("Estimated overall Mean Absolute Error = %0.3f" %(MAE/count))
    return MAE/count
   

def print_most_similar_jokes(dataMat, jokes, queryId, k, metric=pearsonSim):
    ...
    queryJokeVector = dataMat[:,queryId]
    print("Selcted joke: ")
    print(jokes[queryId])
