...
class KVClient (stores: Seq[ActorRef]) {
  ...
  private val cache = new HashMap[BigInt, Any]
  ...
  def write(key: BigInt, value: Any, dirtyset: HashMap[BigInt, Any]) = {
    cache.put(key, value)
    dirtyset.put(key, value)
  }

  /** Push a dirtyset of cached writes through to the server. */
  def push(dirtyset: HashMap[BigInt, Any]) = {
  ...
  private def route(key: BigInt): ActorRef = {
    val temp=(key % stores.length).toInt
    stores(temp)
  }
}
