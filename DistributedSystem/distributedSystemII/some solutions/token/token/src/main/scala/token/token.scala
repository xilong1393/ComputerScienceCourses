...
// END
  ...
  def receive = {
    ...
    // SNAPSHOT: snapshot algorithm messages
    case Marker if inSnapshot => { // SNAPSHOT: not first marker => stop recording from sender 
                                   //                               + print state if done
    ...
    case token:Token => { // SNAPSHOT: first marker => start snapshot
        ...
        if (token == Marker) { // if Marker => close recording from sender
          closed(sender) = true
          numOpen = 1
        } else {              // if Start => do not close recording from sender
          numOpen = 2
        }
